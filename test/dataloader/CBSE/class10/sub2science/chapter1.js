        ...
        it("It should get a board when searched by description", (done) => {
            chai.request('localhost:3001')
                ...
                .end((err, response) => {
                    ...
                    describe("get subjectId Science for Class 10", () => {
                        it("get subjectId Science for Class 10", (done) => {
                            chai.request('localhost:3001')
                                .post("/board/boardId/"+cbseBoardId+"/classId/"+classId10+"/getSubjectIdByName")
                                .set('Content-Type', 'application/json')
                                .set('auth-token', jwtToken)
                                .send({"subjectName" : "Science"})
                                .end((err, response) => {
                                    class10sc = response.body;
                                    response.should.have.status(200);
                                    console.log("this request ran for boardid "+cbseBoardId+" and class id "+classId10)
                                    console.log(class10sc)
                                done();
                                });
                        });
                    });
                    ...
                    describe("add a chapter of a subject", () => {
                        it("It should add a chapter to Science for Class 10", (done) => {
                            chai.request('localhost:3001')
                                .patch("/board/boardId/"+cbseBoardId+"/classId/"+classId10+"/subjectId/"+class10sc+"/addChapter/")
                                .set('Content-Type', 'application/json')
                                .set('auth-token', jwtToken)
                    ...
                    //add questions in bulk
                        describe("add bulk questions to a chapter", () => {
                            it("It should add a question to a chapter", (done) => {
                                chai.request('localhost:3001')
                                    .patch("/board/boardId/"+cbseBoardId+"/classId/"+classId10+"/subjectId/"+class10sc+"/chapterId/"+chapterId1+"/addBulkQuestions/")
                                    .set('Content-Type', 'application/json')
                                    .set('auth-token', jwtToken)
