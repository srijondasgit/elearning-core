        ...
        it("It should get a board when searched by description", (done) => {
            chai.request('localhost:3001')
                ...
                .end((err, response) => {
                    ...
                    describe("get classId for Class 10", () => {
                        it("It should get classId for Class 10", (done) => {
                            chai.request('localhost:3001')
                                .post("/board/boardId/"+cbseBoardId+"/getClassIdByDesc")
                                .set('Content-Type', 'application/json')
                                .set('auth-token', jwtToken)
                                .send({ "classDescription": "Class 10"})
                                .end((err, response) => {
                    ...
                    describe("get subjectId English for Class 10", () => {
                        it("get subjectId English for Class 10", (done) => {
                            chai.request('localhost:3001')
                    ...
                    describe("add a chapter of a subject", () => {
                        it("It should add a chapter to English for Class 10", (done) => {
                            chai.request('localhost:3001')
                                .patch("/board/boardId/"+cbseBoardId+"/classId/"+classId10+"/subjectId/"+class10eng+"/addChapter/")
                                .set('Content-Type', 'application/json')
                                .set('auth-token', jwtToken)
                    ...
                    //         chai.request('localhost:3001')
                    //             .patch("/board/boardId/"+cbseBoardId+"/classId/"+classId10+"/subjectId/"+class10eng+"/chapterId/"+chapterId1+"/addQuestion/")
                    //             .set('Content-Type', 'application/json')
                    //             .set('auth-token', jwtToken)
                    ...
                    //add questions in bulk
                        describe("add bulk questions to a chapter", () => {
                            it("It should add a question to a chapter", (done) => {
                                chai.request('localhost:3001')
                                    .patch("/board/boardId/"+cbseBoardId+"/classId/"+classId10+"/subjectId/"+class10eng+"/chapterId/"+chapterId1+"/addBulkQuestions/")
                                    .set('Content-Type', 'application/json')
                                    .set('auth-token', jwtToken)
