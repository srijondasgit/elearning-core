        ...
        it("It should get a board when searched by description", (done) => {
            chai.request('localhost:3001')
                ...
                .end((err, response) => {
                    ...
                    describe("get subjectId Social Science for Class 6", () => {
                        it("get subjectId Social Science for Class 6", (done) => {
                            chai.request('localhost:3001')
                    ...
                    describe("add a chapter of a subject", () => {
                        it("It should add a chapter to Social Science for Class 6", (done) => {
                            chai.request('localhost:3001')
                                .patch("/board/boardId/"+cbseBoardId+"/classId/"+classId6+"/subjectId/"+class6ss+"/addChapter/")
                                .set('Content-Type', 'application/json')
                                .set('auth-token', jwtToken)
                    ...
                    //         chai.request('localhost:3001')
                    //             .patch("/board/boardId/"+cbseBoardId+"/classId/"+classId6+"/subjectId/"+class6ss+"/chapterId/"+chapterId1+"/addQuestion/")
                    //             .set('Content-Type', 'application/json')
                    //             .set('auth-token', jwtToken)
                    ...
                    //add questions in bulk
                        describe("add bulk questions to a chapter", () => {
                            it("It should add a question to a chapter", (done) => {
                                chai.request('localhost:3001')
                                    .patch("/board/boardId/"+cbseBoardId+"/classId/"+classId6+"/subjectId/"+class6ss+"/chapterId/"+chapterId1+"/addBulkQuestions/")
                                    .set('Content-Type', 'application/json')
                                    .set('auth-token', jwtToken)
