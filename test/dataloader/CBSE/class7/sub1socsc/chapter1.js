        ...
        it("It should get a board when searched by description", (done) => {
            chai.request('localhost:3001')
                ...
                .end((err, response) => {
                    ...
                    describe("get classId for Class 7", () => {
                        it("It should get classId for Class 7", (done) => {
                            chai.request('localhost:3001')
                    ...
                    describe("get subjectId Social Science for Class 7", () => {
                        it("get subjectId Social Science for Class 7", (done) => {
                            chai.request('localhost:3001')
                                .post("/board/boardId/"+cbseBoardId+"/classId/"+classId7+"/getSubjectIdByName")
                                .set('Content-Type', 'application/json')
                                .set('auth-token', jwtToken)
                                .send({"subjectName" : "Social Science"})
                                .end((err, response) => {
                                    class7ss = response.body;
                                    response.should.have.status(200);
                                    console.log("this request ran for boardid "+cbseBoardId+" and class id "+classId7)
                                    console.log(class7ss)
                                done();
                                });
                        });
                    });
                    ...
                    describe("add a chapter of a subject", () => {
                        it("It should add a chapter to Social Science for Class 7", (done) => {
                            chai.request('localhost:3001')
                                .patch("/board/boardId/"+cbseBoardId+"/classId/"+classId7+"/subjectId/"+class7ss+"/addChapter/")
                                .set('Content-Type', 'application/json')
                                .set('auth-token', jwtToken)
                    ...
                    //         chai.request('localhost:3001')
                    //             .patch("/board/boardId/"+cbseBoardId+"/classId/"+classId7+"/subjectId/"+class7ss+"/chapterId/"+chapterId1+"/addQuestion/")
                    //             .set('Content-Type', 'application/json')
                    //             .set('auth-token', jwtToken)
                    ...
                    //add questions in bulk
                        describe("add bulk questions to a chapter", () => {
                            it("It should add a question to a chapter", (done) => {
                                chai.request('localhost:3001')
                                    .patch("/board/boardId/"+cbseBoardId+"/classId/"+classId7+"/subjectId/"+class7ss+"/chapterId/"+chapterId1+"/addBulkQuestions/")
                                    .set('Content-Type', 'application/json')
                                    .set('auth-token', jwtToken)
